...
# data directory for shelby
data_dir <- 'C:/Users/queen/Box/REU Hackathon II/data/'

setwd(data_dir)

first_survey <- read_csv("cleaned_first_survey_on_sexual_and_reproductive_health.csv")

jic_survey <- read_csv("cleaned_jic_sexual_and_reproductive_kit_survey_6-29-2023.csv")

kit_requests <- read_csv("cleaned_kit_requests_F22_S23.csv")

resource_use <- read_csv("cleaned_use_of_srh_and_access_existing_resources_2023.csv")

  
  convenience_orientation <- jic_survey |>
  clean_names() |>
...
n_students <- nrow(resource_use) #Resource Use

resource_use |> # Learn how to reorder the class years
  count(`School Year`) |>
  mutate(prop = n / sum(n)) |>
...
list(label_percent(accuracy = 0.1)(avail_contracept))
```
...
list(value = label_percent(accuracy = 0.1)(cost_barrier),
     color = "#eda25e")
    #value = label_percent(accuracy = 0.1)(cost_barrier)
```
...
access |>
  ...
  mutate(is_access = factor(is_access),
         is_access = fct_relevel(is_access, c("No Access", "Minimum Access", "Uncertain", "Easy Access")),
...
checkboxGroupInput(
  "sexual_orientation",
  "Sexual Orientation",
  levels(factor(convenience_orientation$orientation)),
  selected = levels(factor(convenience_orientation$orientation))
  )

checkboxGroupInput(
  "race",
  "Race/Ethnicity",
  levels(factor(convenience_orientation$race)),
  selected = levels(factor(convenience_orientation$race))
  )

checkboxGroupInput(
  "school_year",
  "Class Year",
  levels(factor(convenience_orientation$school_year)),
  selected = levels(factor(convenience_orientation$school_year))
  )

checkboxGroupInput(
  "gender",
  "Gender",
  levels(factor(convenience_orientation$cleaned_gender)),
  selected = levels(factor(convenience_orientation$cleaned_gender))
  )
...
output$convenience_plot <- renderPlot({
  ...
  convenience_orientation_props <- convenience_orientation |>
...
convenience_orientation_100perc_order <- convenience_orientation_props |>
  filter(convenience == "Very convenient") |>
  arrange(prop) |>
  rowid_to_column(var = "order") |>
  select(location, order)

convenience_orientation_props_100perc <- convenience_orientation_props |>
  left_join(convenience_orientation_100perc_order, by = join_by(location)) |>
  mutate(
  ...
  p <- ggplot(convenience_orientation_props_100perc, aes(y = location, x = prop, fill = convenience)) +
  geom_col() +
